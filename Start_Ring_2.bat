...
echo ============================================
echo   AgilitySoftware - Ring 2 (venv)
...
echo [RING2] Verwende Python 32-bit: %PY32%
echo [RING2] Pruefe virtuelle Umgebung ring_env ...

set "RING_ENV_DIR=%CD%\ring_env"
set "RING_ENV_PY=%RING_ENV_DIR%\Scripts\python.exe"

if not exist "%RING_ENV_PY%" (
  echo [RING2] Erstelle neue venv in ring_env ...
...
echo [RING2] Aktualisiere Pakete in ring_env ...
"%RING_ENV_PY%" -m pip install --upgrade pip
"%RING_ENV_PY%" -m pip install flask flask-socketio requests pywin32

echo [RING2] Starte Ring-Server "%RING_LABEL%" auf Port %RING_PORT% ...
cd ring_server
...
echo [RING2] Ring-Server wurde beendet.
pause
endlocal
exit /b 0
